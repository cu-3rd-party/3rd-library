...
import { defaultFilterState, defaultSortState } from "./defaults";
import { FilterState, SortState } from "./types";

type FilterSortStore = {
  filterState: FilterState;
  sortState: SortState;
  setFilterState: <K extends keyof FilterState>(
    key: K,
    value: FilterState[K],
  ) => void;
  setSortState: <K extends keyof SortState>(
    key: K,
    value: SortState[K],
  ) => void;
    ...
    (set) => ({
      ...
      toggleFilter: (key, newFilter) =>
        set((state) => {
          ...
          const sorted = newValues.sort(
            (a, b) => orderMap[key].indexOf(a) - orderMap[key].indexOf(b),
          );
