...
pub async fn send_verification_code(context: ApiContext, email: String, verification_code: String) -> Result<(), String> {
    let template_html = VERIFICATION_CODE_HTML.get_or_init(|| {
        Html::get("./verification_code.html")
            .and_then(|html| {
                String::from_utf8(html.data.to_vec()).ok()
            }).unwrap_or_default()
        });
    ...
    SmtpClientBuilder::new(context.config.smtp.host.clone(), context.config.smtp.port)?
        .implicit_tls(true)
        .credentials((context.config.smtp.user.clone(), context.config.smtp.password.clone()))
